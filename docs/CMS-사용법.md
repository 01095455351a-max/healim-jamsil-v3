...
**발행하는 날, 「작성일」을 그날로 바꿉니다.** 초안을 써 두었다가 나중에
발행하면 이 칸이 초안 시절 날짜로 남습니다. 홈은 컬럼을 **날짜순 최근 3편**만
싣기 때문에, 날짜가 2주 전이면 오늘 발행해도 홈에 나오지 않습니다.
글 자체는 정상이라 컬럼 목록·게시판·원장 페이지에는 나옵니다 — 그래서
알아채기 어렵습니다. 실제로 산후우울증 컬럼이 이 일로 홈에서 빠졌습니다.

「수정일」도 같이 그날로 맞춥니다. 본문 맨 아래의 **작성일·검토일** 두 줄은
화면에 그대로 보이므로 함께 고칩니다.
